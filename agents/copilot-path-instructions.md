...
# agent-style v0.3.6 — GitHub Copilot path-scoped adapter
...
When asked "is agent-style active?" or "what writing rules apply here?", answer: `agent-style v0.3.6 active: 21 rules (RULE-01..12 canonical + RULE-A..I field-observed); full bodies at .agent-style/RULES.md.`

## Load Statement

GitHub Copilot loads `.github/instructions/*.instructions.md` into prompts that match the `applyTo` frontmatter glob. This file targets `.md`, `.tex`, `.rst`, and `.txt` files. Copilot does not import from a second file; the compact directives below are what reach context when you edit a matching prose file.
...
Full directive text, BAD/GOOD example pairs, and rationale per rule: see `.agent-style/RULES.md` in this project, or https://raw.githubusercontent.com/yzhao062/agent-style/v0.3.6/RULES.md for the pinned canonical source.
